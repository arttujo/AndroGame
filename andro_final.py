...
def weaponsafe(insert):
    ...
    if insert=='3293':
        ...
        if cur.rowcount >= 1:
            ...
            cur.execute(sql2)
            print("You unlock the safe. There seems to be a rifle inside")
...
def deadcheck3(target):
    ...
    if cur.rowcount == 0 and loc=="BRIDGE":
        print("As you land the killing blow with the '"+weapon+"', the alien boss collapses to the floor. You have successfully and nearly single-handedly saved the day.\n"
...
def Escape():
    lause = print("The door closes and you are ejected out of the spaceship in a small box. While drifting aimlessly away from the ship, you soon begin to realise that no one is ever going find you")
    cur = db.cursor()
    sql = "update hero set hero.locID = 'EXIT'"
    cur.execute(sql)

    return lause
def win():
    cur = db.cursor()
    sql = "update hero set locID = 'EXIT2'"
    cur.execute(sql)
    cont=input("\nDo you want to restart the game? [y/n]")
    if cont=='y':
        db.rollback()
...
def lose():
    cur = db.cursor()
    sql = "update hero set locID = 'EXIT'"
    cur.execute(sql)
    cont=input("\nDo you want to restart the game? [y/n]")
    if cont=='y':
        db.rollback()
...
while action!="quit" and loc!="EXIT" or "EXIT2":
    ...
    if loc=="CONTROL":
        ...
        if target=="button" and action=="push" or action=="press":
            Escape()
            newloc = "EXIT"
            loc = newloc
            look_around(loc)
            ...
            print("I don't know what to do with "+target)
